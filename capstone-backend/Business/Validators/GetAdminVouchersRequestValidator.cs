...
{
    public class GetAdminVouchersRequestValidator : AbstractValidator<GetAdminVouchersRequest>
    {
        public GetAdminVouchersRequestValidator()
        {
            ...
            RuleFor(x => x.VenueOwnerId)
                .GreaterThan(0).WithMessage("VenueOwnerId phải lớn hơn 0")
                .When(x => x.VenueOwnerId.HasValue);

            RuleFor(x => x.Status)
                .Must(status => !status.HasValue || status != VoucherStatus.DRAFTED)
                .WithMessage("Không thể lọc voucher theo trạng thái DRAFTED");
