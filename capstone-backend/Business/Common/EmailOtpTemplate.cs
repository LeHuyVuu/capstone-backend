...
";
    ...
    /// <summary>
    /// Generate HTML email template cho OTP xác thực đăng ký tài khoản
    /// </summary>
    /// <param name="otpCode">Mã OTP 6 chữ số</param>
    /// <param name="email">Email đăng ký</param>
    /// <returns>HTML email content</returns>
    public static string GetRegistrationOtpEmail(string otpCode, string email)
    {
        return $@"
<!DOCTYPE html>
<html>
<head>
    <meta charset=""UTF-8"">
    <meta name=""viewport"" content=""width=device-width, initial-scale=1.0"">
    <title>Xác thực đăng ký - CoupleMood</title>
</head>
...
<table width=""600"" cellpadding=""0"" cellspacing=""0"" style=""max-width:600px;background:#ffffff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);"">
    ...
    <tr>
        <td style=""padding:30px 30px 20px 30px;"">
            <p style=""margin:0;font-size:16px;color:#111827;line-height:1.6;"">
                Xin chào <strong style=""color:#667eea;"">{email}</strong>,
            </p>
            <p style=""margin:12px 0 0 0;color:#4b5563;font-size:15px;line-height:1.6;"">
                Cảm ơn bạn đã đăng ký tài khoản CoupleMood. 
                Vui lòng sử dụng mã OTP bên dưới để xác thực email và hoàn tất đăng ký:
            </p>
        </td>
    </tr>
    ...
    /// <summary>
    /// Generate plain text email cho OTP xác thực đăng ký (fallback)
    /// </summary>
    /// <param name="otpCode">Mã OTP 6 chữ số</param>
    /// <returns>Plain text email content</returns>
    public static string GetRegistrationOtpPlainText(string otpCode)
    {
        return $@"
Xin chào,
